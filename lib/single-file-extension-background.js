(function () {
	...
	async function createMenus(tab) {
		...
		let options = await getOptions(tab && tab.url);
		if (BROWSER_MENUS_API_SUPPORTED && options) {
			...
			if (options.profileName == DISABLED_PROFILE_NAME) {
				options = await getOptions();
				options.profileName = DISABLED_PROFILE_NAME;
			}
	...
	const fetch$2 = window.fetch.bind(window);
	...
	async function onFetchFrame(message) {
		try {
			const response = await fetch$2(message.url, { cache: "force-cache", headers: message.headers, referrerPolicy: "strict-origin-when-cross-origin" });
			return {
				status: response.status,
				headers: [...response.headers],
		...
		try {
			...
			try {
				if ((options.referrer && !USE_HOST_FETCH) || !useHostFetch) {
					response = await fetch$2(url, fetchOptions);
				} else {
	...
	async function saveUrls(urls, options = {}) {
		...
		await Promise.all(urls.map(async url => {
			...
			if (tabOptions.profileName != DISABLED_PROFILE_NAME) {
	...
	const CONFLICT_ACTION_UNIQUIFY$6 = "uniquify";
	const CONFLICT_ACTION_OVERWRITE$5 = "overwrite";
	const CONFLICT_ACTION_SKIP$5 = "skip";
	const CONFLICT_ACTION_PROMPT$5 = "prompt";
	...
	let MediaUploader$1 = class MediaUploader {
		constructor(options) {
		...
		async upload(indexFilename = 1) {
			...
			if (response.files.length) {
				if (this.filenameConflictAction == CONFLICT_ACTION_OVERWRITE$5) {
					method = "PATCH";
					fileId = response.files[0].id;
					this.metadata.parents = null;
				} else if (this.filenameConflictAction == CONFLICT_ACTION_UNIQUIFY$6) {
				...
				} else if (this.filenameConflictAction == CONFLICT_ACTION_PROMPT$5) {
					if (this.prompt) {
						...
						this.filenameConflictAction = CONFLICT_ACTION_UNIQUIFY$6;
						return this.upload(indexFilename);
					}
				} else if (this.filenameConflictAction == CONFLICT_ACTION_SKIP$5) {
					return response;
				}
			}
	...
	const CONFLICT_ACTION_UNIQUIFY$5 = "uniquify";
	const CONFLICT_ACTION_OVERWRITE$4 = "overwrite";
	const CONFLICT_ACTION_SKIP$4 = "skip";
	const CONFLICT_ACTION_PROMPT$4 = "prompt";
	...
	class MediaUploader {
		...
		async upload() {
			...
			if (response.matches.length) {
				if (this.filenameConflictAction == CONFLICT_ACTION_PROMPT$4) {
					if (this.prompt) {
						const name = await this.prompt(this.metadata.name);
						if (name) {
							this.metadata.name = name;
						...
						this.filenameConflictAction = CONFLICT_ACTION_UNIQUIFY$5;
					}
	...
	async function sendFile(mediaUploader) {
		...
		const httpFinishResponse = await fetch(DROPBOX_FINISH_URL, {
			...
			headers: {
				...
				"Dropbox-API-Arg": stringify({
					...
					commit: {
						path,
						mode: mediaUploader.filenameConflictAction == CONFLICT_ACTION_OVERWRITE$4 ? "overwrite" : "add",
						autorename: mediaUploader.filenameConflictAction == CONFLICT_ACTION_UNIQUIFY$5
					}
				})
			}
		});
		...
		} else if (httpFinishResponse.status == 409 && mediaUploader.filenameConflictAction == CONFLICT_ACTION_PROMPT$4) {
	...
	const EXTENSION_SEPARATOR$3 = ".";
	const ERROR_PREFIX_MESSAGE = "Error ";
	const INDEX_FILENAME_PREFIX$3 = " (";
	const INDEX_FILENAME_SUFFIX$3 = ")";
	const INDEX_FILENAME_REGEXP$3 = /\s\((\d+)\)$/;
	const ABORT_ERROR_NAME$3 = "AbortError";
	const HEAD_METHOD$1 = "HEAD";
	const PUT_METHOD$2 = "PUT";
	...
	class WebDAV {
		...
		upload(filename, content, options) {
			...
			return upload$2(filename, content, options);
		}
	...
	async function upload$2(filename, content, options) {
		const { authorization, filenameConflictAction, prompt, signal, preventRetry } = options;
		let { url } = options;
		try {
			if (filenameConflictAction == CONFLICT_ACTION_OVERWRITE$3) {
				let response = await sendRequest(filename, PUT_METHOD$2, content);
				if (response.status == CREATED_STATUS) {
					...
					return await upload$2(filename, content, options);
				}
			} else {
				let response = await sendRequest(filename, HEAD_METHOD$1);
				if (response.status == FOUND_STATUS) {
					if (filenameConflictAction == CONFLICT_ACTION_UNIQUIFY$4 || (filenameConflictAction == CONFLICT_ACTION_PROMPT$3 && !prompt)) {
						const { filenameWithoutExtension, extension, indexFilename } = splitFilename(filename);
						options.indexFilename = indexFilename + 1;
						return await upload$2(getFilename(filenameWithoutExtension, extension), content, options);
					} else if (filenameConflictAction == CONFLICT_ACTION_PROMPT$3) {
						filename = await prompt(filename);
						return filename ? upload$2(filename, content, options) : response;
					} else if (filenameConflictAction == CONFLICT_ACTION_SKIP$3) {
						return response;
					}
				} else if (response.status == NOT_FOUND_STATUS) {
					response = await sendRequest(filename, PUT_METHOD$2, content);
			...
			if (error.name != ABORT_ERROR_NAME$3) {
				throw error;
			}
		}
		...
		function sendRequest(path, method, body) {
			...
			if (body) {
				headers[CONTENT_TYPE_HEADER$1] = HTML_CONTENT_TYPE;
			}
		...
		function splitFilename(filename) {
			...
			let extension = EMPTY_STRING$3;
			const indexExtensionSeparator = filename.lastIndexOf(EXTENSION_SEPARATOR$3);
		...
		function extractIndexFilename(filenameWithoutExtension) {
			const indexFilenameMatch = filenameWithoutExtension.match(INDEX_FILENAME_REGEXP$3);
			let indexFilename = 0;
			if (indexFilenameMatch && indexFilenameMatch.length > 1) {
				...
				if (!Number.isNaN(parsedIndexFilename)) {
					...
					filenameWithoutExtension = filenameWithoutExtension.replace(INDEX_FILENAME_REGEXP$3, EMPTY_STRING$3);
				}
			}
		...
		function getFilename(filenameWithoutExtension, extension) {
			return filenameWithoutExtension +
				INDEX_FILENAME_PREFIX$3 + options.indexFilename + INDEX_FILENAME_SUFFIX$3 +
				(extension ? EXTENSION_SEPARATOR$3 + extension : EMPTY_STRING$3);
		}

		async function createDirectories() {
			const filenameParts = filename.split(DIRECTORY_SEPARATOR);
			filenameParts.pop();
			let path = EMPTY_STRING$3;
	...
	const EXTENSION_SEPARATOR$2 = ".";
	const INDEX_FILENAME_PREFIX$2 = " (";
	const INDEX_FILENAME_SUFFIX$2 = ")";
	const INDEX_FILENAME_REGEXP$2 = /\s\((\d+)\)$/;
	const ABORT_ERROR_NAME$2 = "AbortError";
	const GET_METHOD$1 = "GET";
	...
	class GitHub {
		...
		async upload(path, content, options) {
			...
			return upload$1(this.userName, this.repositoryName, this.branch, path, base64Content, options);
		}
	...
	async function upload$1(userName, repositoryName, branch, path, content, options) {
		...
		async function createContent({ path, content, message = EMPTY_STRING$2, sha }) {
			try {
				const response = await fetchContentData(PUT_METHOD$1, JSON.stringify({
					content,
					message,
					branch,
					sha
				...
				if (response.status == 422) {
					if (filenameConflictAction == CONFLICT_ACTION_OVERWRITE$2) {
						const response = await fetchContentData(GET_METHOD$1);
						const responseData = await response.json();
						const sha = responseData.sha;
						return await createContent({ path, content, message, sha });
					} else if (filenameConflictAction == CONFLICT_ACTION_UNIQUIFY$3) {
						...
						return responseData;
					} else if (filenameConflictAction == CONFLICT_ACTION_PROMPT$2) {
						if (prompt) {
							...
							options.filenameConflictAction = CONFLICT_ACTION_UNIQUIFY$3;
							return await createContent({ path, content, message });
						}
					}
				}
				...
				if (error.name != ABORT_ERROR_NAME$2) {
					throw error;
				}
			}
		...
		function splitFilename(filename) {
			let filenameWithoutExtension = filename;
			let extension = EMPTY_STRING$2;
			const indexExtensionSeparator = filename.lastIndexOf(EXTENSION_SEPARATOR$2);
			if (indexExtensionSeparator > -1) {
				filenameWithoutExtension = filename.substring(0, indexExtensionSeparator);
				extension = filename.substring(indexExtensionSeparator + 1);
			}
		...
		function extractIndexFilename(filenameWithoutExtension) {
			const indexFilenameMatch = filenameWithoutExtension.match(INDEX_FILENAME_REGEXP$2);
			let indexFilename = 0;
			if (indexFilenameMatch && indexFilenameMatch.length > 1) {
				const parsedIndexFilename = Number(indexFilenameMatch[indexFilenameMatch.length - 1]);
				if (!Number.isNaN(parsedIndexFilename)) {
					indexFilename = parsedIndexFilename;
					filenameWithoutExtension = filenameWithoutExtension.replace(INDEX_FILENAME_REGEXP$2, EMPTY_STRING$2);
				}
			}
			return { filenameWithoutExtension, indexFilename };
		}
		...
		function getFilename(filenameWithoutExtension, extension) {
			return filenameWithoutExtension +
				INDEX_FILENAME_PREFIX$2 + options.indexFilename + INDEX_FILENAME_SUFFIX$2 +
				(extension ? EXTENSION_SEPARATOR$2 + extension : EMPTY_STRING$2);
		}
	}
			...
			try {
				if (filenameConflictAction == CONFLICT_ACTION_OVERWRITE$1) {
					...
					if (response.status == 200) {
						if (filenameConflictAction == CONFLICT_ACTION_PROMPT$1) {
							if (prompt) {
								path = await prompt(path);
								if (path) {
								...
								options.filenameConflictAction = CONFLICT_ACTION_UNIQUIFY$2;
								return this.upload(path, blob, options);
							}
						} else if (filenameConflictAction == CONFLICT_ACTION_UNIQUIFY$2) {
							const { filenameWithoutExtension, extension, indexFilename } = splitFilename$1(path);
							options.indexFilename = indexFilename + 1;
							path = getFilename$1(filenameWithoutExtension, options.indexFilename, extension);
							return this.upload(path, blob, options);
						}
	...
	function splitFilename$1(filename) {
		...
		let indexFilename;
		({ filenameWithoutExtension, indexFilename } = extractIndexFilename$1(filenameWithoutExtension));
		return { filenameWithoutExtension, extension, indexFilename };
	}

	function extractIndexFilename$1(filenameWithoutExtension) {
	...
	function getFilename$1(filenameWithoutExtension, indexFilename, extension) {
		return filenameWithoutExtension +
			INDEX_FILENAME_PREFIX$1 + indexFilename + INDEX_FILENAME_SUFFIX$1 +
	...
	async function upload(path, content, options) {
	    ...
	    try {
	        ...
	        if (existsResponse.exists) {
	            if (filenameConflictAction == CONFLICT_ACTION_SKIP$1) {
		...
		try {
			...
			if (skipped) {
				...
				if (message.openEditor) {
					onEdit(tabId);
					await open({ tabIndex: tab.index + 1, filename: message.filename, content: contents.join(""), url: message.originalUrl });
	...
	async function saveContent(message, tab) {
		...
		if (options) {
			...
			try {
				...
				options.tabId = tabId;
				pageData = await getPageData(options, { fetch: fetch$1 }, null, null);
				let skipped;
				if (!options.saveToGDrive && !options.saveWithWebDAV && !options.saveWithMCP && !options.saveToGitHub && !options.saveToDropbox && !options.saveWithCompanion && !options.saveToRestFormApi && !options.saveToS3) {
					...
					if (!skipped) {
						...
						if (options.saveToGDrive) {
							...
							await saveWithMCP(message.taskId, encodeSharpCharacter(pageData.filename), content, options.mcpServerUrl, options.mcpAuthToken, {
								filenameConflictAction: options.filenameConflictAction
							});
